...
/**
 * Service for calculating priority based on various factors including creation date Implements
 * age-based priority to prevent starvation and ensure fairness
 */
@Service
@Slf4j
public class PriorityCalculationService {
    ...
    /**
     * Calculate priority based on age (time since creation) and business logic Higher priority for
    ...
    public int calculateAgeBasedPriority(LocalDateTime createdAt) {
        ...
        if (hoursWaited >= 24)
            return 20; // Chờ > 24h: Priority cao nhất
        if (hoursWaited >= 12)
            return 16; // Chờ > 12h: Priority cao
        if (hoursWaited >= 6)
            return 12; // Chờ > 6h: Priority trung bình cao
        if (hoursWaited >= 2)
            return 8; // Chờ > 2h: Priority trung bình
        if (hoursWaited >= 1)
            return 4; // Chờ > 1h: Priority thấp
    ...
    public int calculateHybridPriority(LocalDateTime createdAt, int basePriority,
        ...
        // Combine: 60% age + 40% business
        return (int) (agePriority * 0.6 + businessPriority * 0.4);
    }

    /**
     * Calculate priority with human-like factor consideration Non-human requests get priority boost
     * for faster processing
     */
    public int calculatePriorityWithHumanFactor(LocalDateTime createdAt, boolean isHumanLike,
            boolean isHighValue, boolean isUrgent) {
        int basePriority = calculateHybridPriority(createdAt, 0, isHighValue, isUrgent);

        // Non-human requests get significant priority boost (faster processing)
        if (!isHumanLike) {
            basePriority += 5; // Increased from 2 to 5 for stronger preference
        }
